...
class ScaleTransform(Transform):
    ...
    def reverse(self, x):
        ...
        x = ensure_2d(x)
        delta = self.maxima - self.minima + EPSILON
        return (x * delta + self.minima).copy()

    def forward_std(self, x_std):
        assert self._is_fit
        if x_std is None:
            return None
        x_std = ensure_2d(x_std)
        delta = self.maxima - self.minima + EPSILON
        return (x_std / delta).copy()
...
class NormalizeTransform(Transform):
    ...
    def forward(self, x):
        ...
        return ((x - self.mean) / denominator).copy()
    ...
    def reverse(self, x):
        ...
        x = ensure_2d(x)
        denominator = self.std + EPSILON
        return (x * denominator + self.mean).copy()
